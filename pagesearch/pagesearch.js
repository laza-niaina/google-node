...
class PageSearch extends Requests {
  ...
  async nextResults() {
    if (!this.results.end) {
      this.page += 1;
      const nextResults = await this.__getResults();
  ...
  async __getResults() {
    ...
    const { relevantResult, relatedQuestions, results } = parseHtmlResult(cleanedHtmlResult);
  ...
  async __fetchRawResult() {
    ...
    const response = await this.get(this.url, { params: requestBody, allowRedirects: true });

    if (response.status !== 200) {
      throw new Error(`Failed to fetch raw results. ${response.status} ${response.reason}`);
    }
