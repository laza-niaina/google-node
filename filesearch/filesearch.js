...
const { GoogleSearch, Requests, extractHtmlBlock } = require('../utils');

class FileSearch extends GoogleSearch {
  constructor(query, filetype, limit = 10, page = 1, lang = 'fr', region = 'fr', safeSearch = false) {
    super(query, lang, region, safeSearch);

    this.session.defaults.headers.common['Accept-Language'] = `${LANGUAGES[lang]},${lang};q=0.5`;
    this.session.defaults.headers.common['User-Agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/117.0';
    this.session.defaults.headers.common['Accept'] = '*/*';
    this.session.defaults.headers.common['Accept-Encoding'] = 'gzip, deflate, br';
  ...
  __fetchRawResult() {
    ...
    return this.get(this.url, { params: requestBody, allowRedirects: true })
      .then((response) => {
        if (response.status !== 200) {
          throw new Error(`Failed to fetch raw results. ${response.status} ${response.statusText}`);
        }
